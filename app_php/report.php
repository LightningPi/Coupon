...
$report_content=$_GET['report_content'];


//echo "帳號：".$ac."密碼：".$pw;


$sql="INSERT INTO report(name,title,content,email,type) VALUES ('".$report_name."','".$report_title."','".$report_content."','".$report_email."','2')";

mysql_query($sql)or die("Query error:".mysql_error());  //設定$result為mysql的傳送語法，並將$sql中的SELECT語法代入

	$reportdone[]=array("reportgood" => "回報成功");	//把 「錯誤訊息」 設定成「陣列變數」。

	echo $_GET['jsoncallback'].'('.json_encode($reportdone).');';  //$_GET裡的jsoncallback會在login.html裡用到。$wrong的資料是php array，透過json_encode轉換成json可傳送的值。
