...
$sql="SELECT coupon.coupon_id,coupon.store_id,store.name,coupon.title,coupon.content,coupon.timestart,coupon.timeend,coupon.pic,store.time,store.tel FROM coupon CROSS JOIN store WHERE coupon.store_id=store.store_id ORDER BY RAND() LIMIT 1"; 

  //從優惠券資料表coupon，隨機撈取一筆優惠券
...
echo $_GET['jsoncallback'].'('.json_encode($record).');';  //$_GET裡的jsoncallback會在eat_result.html裡用到。$record的資料是php array，透過json_encode轉換成json可傳送的值。
?>
