...
$store_id=$_GET['id'];		//商家主鍵，變數$store_id，接收由long.html傳送的id

$sql="SELECT content FROM coupon WHERE store_id=".$store_id." && class=1";

//長期優惠class = 1，從優惠券資料表coupon，撈取符合商家主鍵的長期優惠
...
echo $_GET['jsoncallback'].'('.json_encode($record).');';  //$_GET裡的jsoncallback會在long.html裡用到。$record的資料是php array，透過json_encode轉換成json可傳送的值。
?>
